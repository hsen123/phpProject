...
class AnalysisSubscriber implements EventSubscriberInterface
{
    ...
    {
        return [
            KernelEvents::VIEW => ['processNewAnalysisEntity', EventPriorities::PRE_VALIDATE]
        ];
    }

    const ADD_RESULT_IDS_KEY = "addResultIds";
    const REMOVE_RESULT_IDS_KEY = "removeResultIds";

    public function processNewAnalysisEntity(GetResponseForControllerResultEvent $event)
    {
    ...
    private function processUpdateAnalysis(GetResponseForControllerResultEvent $event, Analysis $analysis)
    {
        ...
        if (isset($json["discarded"])) {
            ...
            if ($isNotAdmin && false === $value && true === $analysis->getDiscarded()) {
                ...
                foreach ($analysis->getDynamicShares()->getIterator() as $dynamicShare) {
                    $this->entityManager->remove($dynamicShare);
                }
